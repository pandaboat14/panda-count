// Panda Diplomacy battle codex: every number and word the Pokémon-style battles run on.
// A typed port of design/battles/shared/codex-data.js. Units, heroes, costs and building names match ../rules.ts.
import type {
  BuildingId,
  Cost,
  DoctrineDef,
...
export const GOODS: Record<GoodId, { label: string; icon: string; color: string }> = {
...
export const TYPES: Record<TypeId, { label: string; icon: string; color: string; ink: string; beats: TypeId | null; why: string }> = {
...
export const SUPER = 1.5;
export const RESIST = 0.75;
export const LEGEND_TAKEN = 0.75;
export function typeMult(atkType: TypeId, defType: TypeId) {
...
export const UNITS: Record<UnitId, { label: string; plural: string; icon: string; type: TypeId; hp: number; atk: number; def: number; cost: Cost; blurb: string }> = {
...
export const HEROES: Record<HeroId, { name: string; title: string; icon: string; type: TypeId; hp: number; atk: number; def: number; aura: number; blurb: string }> = {
...
type MoveInput = Omit<MoveDef, "id" | "bonus" | "power" | "cost" | "tags" | "effects"> & Partial<Pick<MoveDef, "bonus" | "power" | "cost" | "tags" | "effects">>;
const M = <I extends string>(id: I, o: MoveInput): MoveDef & { id: I } => ({ id, bonus: 0, power: 0, cost: {}, tags: [], effects: [], ...o });
...
export const STATUSES: Record<StatusId, { label: string; icon: string; good: boolean | null; text: string }> = {
...
export const TERRAIN: Record<TerrainId, { label: string; icon: string; element: TerrainId; rule: string; text: string }> = {
...
export const BUILDINGS: Record<BuildingId, { label: string; icon: string; text: string }> = {
...
export const EVENTS: Record<WorldEventId, { label: string; icon: string; text: string }> = {
...
export const NATIVE_DOCTRINE: Record<NativeId, DoctrineId> = { pandas: "turtle", nacams: "berserk", cams: "showboat", wild: "skittish" };
export const NATIVE_NAMES: Record<NativeId, string> = { pandas: "the Panda Nation", nacams: "the NACAM Ogre Nation", cams: "the CAM Nation", wild: "the wild pandas" };
...
export const CODEX = { GOODS, TYPES, SUPER, RESIST, LEGEND_TAKEN, typeMult, UNITS, HEROES, MOVES, MOVE_BY_ID, WEAPONS, ITEMS, STATUSES, TERRAIN, BUILDINGS, EVENTS, DOCTRINES, NATIVE_DOCTRINE, NATIVE_NAMES, RULES };
